                ...
                for childJob in dag.getJobList():
                    if childJob.getId() == childId:
                        for outFile in parentJob.getUseList():
                            if outFile.getLink() == DAG.LinkageType.OUTPUT:
                                for inFile in childJob.getUseList():
                                    if inFile.getLink() == DAG.LinkageType.INPUT and inFile.getFile() == outFile.getFile():
    ...
    def convertDagToWorkflowGraph(self, dag):
        ...
        for job in dag.getJobList():
            ...
            for file in job.getUseList():
                if file.getLink() == DAG.LinkageType.INPUT and int(file.getSize()) > 0 :
                    inputFilesSize += int(file.getSize())
                elif file.getLink() == DAG.LinkageType.OUTPUT and int(file.getSize()) > 0:
            ...
            wfNode.setInputFileSize(runTime * Constants().STANDARD_MIPS)
        ...
        for child in dag.getChildList():
            ...
            for parent in child.getParentList():
                ...
                self.nodes.get(childId).addParent(parent.getRef(), IOsize) # you can make it two part if didn't work
