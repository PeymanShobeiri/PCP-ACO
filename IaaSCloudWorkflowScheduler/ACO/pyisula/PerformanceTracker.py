...
class PerformanceTracker(Environment):
    ...
    def isStateValid(self, ant, environment):
        if self.__bestSolution == None and self.__bestSolutionAsString == "" and self.__bestSolutionCost == 0.0:
            return True
    ...
    def updateIterationPerformance(self, antColony, iteration, iterationTimeInSeconds, environment):
        ...
        if not self.isStateValid(bestAnt, environment):
            raise SolutionConstructionException("Performance Tracker is in an inconsistent state. Solution:"+ \
                    self.__bestSolution + " Solution Cost: " + self.__bestSolutionCost + " String representation: " +\
                    self.__bestSolutionAsString + " Colony index: " + antColony.getColonyIndex())
        ...
        logger.info(" Global solution cost: " + self.__bestSolutionCost + " Stored solution: " + self.__bestSolution + " Solution as String: " + self.__bestSolutionAsString)
