...
class PcpPolicyInsertion(WorkflowPolicy):
    ...
    def updateChildrenEST(self, parentNode):
        for child in parentNode.getChildren():
            ...
            if childNode.getEST() < newEST:
                ...
                #############
                if childNode.isScheduled() and self._instances.getInstance(childNode.getSelectedResource()).getFinishTime() < childNode.getEFT():
    ...
    def assignPath(self, path):
        ...
        for curInst in range(len(self._instances.getSize())):
